...
class sk{
  ...
  int begin( int pin, int count){
    Serial.printf("sk driver has begun. pin: %d, count: %d\n", pin, count);
    _bitcount = count*32;
    _ledpin = pin;
    _ledcount = count;
    _brightness = 100;
  ...
  *w = kleur&0xff;
  ...
  void show(){
      ...
      //Serial.println("Well I think I found where the error is coming from");
      //Serial.printf("sk's channel: %d, _ledcount: %d, _ledpin: %d\n", skconfig.channel, _ledcount, _ledpin);
